(()=>{
  const init=()=>{
    ...
    const savedApi=window.KYOKAI_SAVED_STORIES;
    ...
    for(const [id,href] of Object.entries(seriesPages)){
      const card=document.getElementById(id);const head=card?.querySelector('.series-card-head');
      if(!head||head.querySelector('.series-detail-link'))continue;
      const link=document.createElement('a');link.className='series-detail-link';link.href=href;link.textContent='シリーズの案内と全話一覧';head.appendChild(link);
    }

    const tools=document.createElement('div');tools.className='archive-tools';tools.setAttribute('aria-label','作品を検索・絞り込み・並べ替え');
    const search=document.createElement('input');search.type='search';search.placeholder='作品名・題材・あらすじ・IDで検索';search.setAttribute('aria-label','作品検索');
    const series=document.createElement('select');series.setAttribute('aria-label','シリーズで絞り込み');
    ['すべてのシリーズ',...Object.keys(window.KYOKAI_SERIES||{})].forEach((name,index)=>{const option=document.createElement('option');option.value=index===0?'':name;option.textContent=name;series.appendChild(option);});
    const modes=[{value:'',label:'すべての読み方'},{value:'quick',label:'6分以内'},{value:'fear5',label:'恐怖度5'},{value:'standalone',label:'単独で読みやすい'},{value:'serial',label:'連作を順番に'},{value:'long',label:'10分以上'}];
    const mode=document.createElement('select');mode.setAttribute('aria-label','読み方で絞り込み');modes.forEach(item=>{const option=document.createElement('option');option.value=item.value;option.textContent=item.label;mode.appendChild(option);});
    const readOptions=[{value:'',label:'未読・読了すべて'},{value:'unread',label:'未読だけ'},{value:'read',label:'読了済みだけ'}];
    const read=document.createElement('select');read.className='read-status-select';read.setAttribute('aria-label','読了状態で絞り込み');readOptions.forEach(item=>{const option=document.createElement('option');option.value=item.value;option.textContent=item.label;read.appendChild(option);});
    const savedOptions=[{value:'',label:'保存状態すべて'},{value:'saved',label:'あとで読むだけ'}];
    const saved=document.createElement('select');saved.className='save-status-select';saved.dataset.role='saved';saved.setAttribute('aria-label','あとで読むの保存状態で絞り込み');savedOptions.forEach(item=>{const option=document.createElement('option');option.value=item.value;option.textContent=item.label;saved.appendChild(option);});
    const sorts=[{value:'',label:'公開順'},{value:'short',label:'短い順'},{value:'long',label:'長い順'},{value:'fear',label:'恐怖度が高い順'}];
    const sort=document.createElement('select');sort.setAttribute('aria-label','作品の並べ替え');sorts.forEach(item=>{const option=document.createElement('option');option.value=item.value;option.textContent=item.label;sort.appendChild(option);});
    ...
    const reset=document.createElement('button');reset.type='button';reset.className='archive-reset';reset.textContent='条件をリセット';actions.append(nextUnread,random,reset);
    const result=document.createElement('p');result.className='archive-result';result.setAttribute('aria-live','polite');
    tools.append(search,series,mode,read,saved,sort,actions,result);grid.parentNode.insertBefore(tools,grid);
    ...
    let visibleEntries=[...entries];let unreadCandidates=[...entries];
    ...
    const isSaved=work=>Boolean(savedApi?.isSaved(work.id));
    const matchesMode=(work,value)=>{if(!value)return true;if(value==='quick')return minutes(work)<=6;if(value==='fear5')return Number(work.fear)===5;if(value==='standalone')return work.series!=='境界観測記';if(value==='serial')return work.series==='境界観測記';if(value==='long')return minutes(work)>=10;return true;};
    const matchesRead=(work,value)=>!value||(value==='read'?isRead(work):!isRead(work));
    const matchesSaved=(work,value)=>!value||(value==='saved'&&isSaved(work));
    ...
    const savedLabel=value=>savedOptions.find(item=>item.value===value)?.label||'';
    const orderEntries=list=>{const ordered=[...list];if(sort.value==='short')ordered.sort((a,b)=>minutes(a.work)-minutes(b.work)||a.index-b.index);else if(sort.value==='long')ordered.sort((a,b)=>minutes(b.work)-minutes(a.work)||a.index-b.index);else if(sort.value==='fear')ordered.sort((a,b)=>Number(b.work.fear)-Number(a.work.fear)||minutes(a.work)-minutes(b.work)||a.index-b.index);else ordered.sort((a,b)=>a.index-b.index);return ordered;};
    ...
    const initialSaved=params.get('saved')||'';if(savedOptions.some(item=>item.value===initialSaved))saved.value=initialSaved;
    const initialSort=params.get('sort')||'';if(sorts.some(item=>item.value===initialSort))sort.value=initialSort;search.value=params.get('q')||'';
    const syncUrl=()=>{if(!history.replaceState)return;const next=new URL(location.href);const values={pick:mode.value,series:series.value,read:read.value,saved:saved.value,sort:sort.value,q:search.value.trim()};for(const [key,value] of Object.entries(values)){if(value)next.searchParams.set(key,value);else next.searchParams.delete(key);}history.replaceState(null,'',`${next.pathname}${next.search}${next.hash}`);};
    const apply=()=>{
      const query=normalize(search.value).trim();const selectedSeries=series.value;
      const baseEntries=entries.filter(({work,card})=>{const text=normalize(`${work.id} ${work.title} ${work.desc} ${work.series} ${card.dataset.search||''} ${card.dataset.tags||''} ${card.dataset.format||''}`);return (!selectedSeries||work.series===selectedSeries)&&matchesMode(work,mode.value)&&(!query||text.includes(query));});
      visibleEntries=orderEntries(baseEntries.filter(({work})=>matchesRead(work,read.value)&&matchesSaved(work,saved.value)));
      unreadCandidates=orderEntries(baseEntries.filter(({work})=>!isRead(work)&&matchesSaved(work,saved.value)));
      orderEntries(entries).forEach(entry=>grid.appendChild(entry.card));const visibleSet=new Set(visibleEntries);entries.forEach(entry=>{entry.card.hidden=!visibleSet.has(entry);});
      const labels=[];if(mode.value)labels.push(modeLabel(mode.value));if(selectedSeries)labels.push(selectedSeries);if(read.value)labels.push(readLabel(read.value));if(saved.value)labels.push(savedLabel(saved.value));if(query)labels.push(`「${search.value.trim()}」`);
      result.textContent=`${visibleEntries.length}話を表示中（全${works.length}話）${labels.length?`・条件: ${labels.join(' / ')}`:''}`;
      nextUnread.disabled=!unreadCandidates.length;nextUnread.textContent=unreadCandidates.length?`次の未読作品（${unreadCandidates.length}話）`:'条件内はすべて読了済み';syncUrl();
    };
    search.addEventListener('input',apply);series.addEventListener('change',apply);mode.addEventListener('change',apply);read.addEventListener('change',apply);saved.addEventListener('change',apply);sort.addEventListener('change',apply);
    ...
    reset.addEventListener('click',()=>{search.value='';series.value='';mode.value='';read.value='';saved.value='';sort.value='';apply();search.focus();});
    document.addEventListener('kyokai-reading-status-change',apply);document.addEventListener('kyokai-saved-stories-change',apply);apply();
  };
