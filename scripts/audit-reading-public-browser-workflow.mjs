...
const requiredWorkflow=[
  ...
  ['tests/reading-backup.spec.mjs tests/reading-state.spec.mjs tests/reading-discovery.spec.mjs','バックアップ・通常読書・検索個別化の同時試験'],
  ['--retries=2','公開試験の再試行'],
...
for(const watched of ['index.html','series/*.html','data/archive-tools.js','data/series-archive-tools.js','data/home-personalization.js','tests/reading-discovery.spec.mjs'])if(!workflow.includes(`- '${watched}'`))errors.push(`検索・個別化監査の変更監視がありません: ${watched}`);
