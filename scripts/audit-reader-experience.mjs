...
const cssRequirements = [
  ...
  ['前後話リンクのタップ領域', 'min-height:52px'],
];
for (const [label, token] of cssRequirements) if (!css.includes(token)) errors.push(`reader-tools.css: ${label}がありません`);
...
for (const work of works) {
  ...
  const styleCount = (html.match(/href=["']\/kyokai-yawa\/data\/reader-tools\.css["']/g) || []).length;
  const scriptCount = (html.match(/src=["']\/kyokai-yawa\/data\/reader-tools\.js["']/g) || []).length;
  const articleMatch = html.match(/<article\b[^>]*id=["']story["'][^>]*>([\s\S]*?)<\/article>/i);
  const navMatch = html.match(/<nav\b[^>]*class=["'][^"']*footer-nav[^"']*["'][^>]*>([\s\S]*?)<\/nav>/i);
  const paragraphs = articleMatch ? (articleMatch[1].match(/<p\b/gi) || []).length : 0;
  const navLinks = navMatch ? (navMatch[1].match(/<a\b/gi) || []).length : 0;
  ...
  if (!articleMatch) errors.push(`${work.id}: article#storyがありません`);
  if (!/<div\b[^>]*class=["'][^"']*meta[^"']*["']/i.test(html)) errors.push(`${work.id}: 作品情報meta領域がありません`);
  if (!navMatch) errors.push(`${work.id}: 前後話ナビゲーションがありません`);
...
const reportPath = path.join(root, 'reports', 'reader-experience-audit.md');
