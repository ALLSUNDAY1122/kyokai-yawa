...
const configs={
  '真壁夜話':{
    slug:'makabe',english:'MAKABE NIGHT RECORDS',
    lead:'勤怠、入館、予約、決済。日常を支える業務記録が、現実より先に結果を確定していく現代怪談。',
    flavor:'職場・設備・情報システム',start:'MKB-001「退勤記録 25:14」',
    order:'各話は独立。気になる題名から読めます。公開順では、記録が人間を従わせる規則の変化も追えます。',
    promise:'怪異の規則を確認し、記録と現実のずれを具体的な手順で閉じる一話完結型です。'
  },
  '黒瀬蒐集録':{
    slug:'kurose',english:'KUROSE COLLECTION',
    lead:'古地図、台帳、祭礼記録、土地の呼び名。残された資料を照合し、地域に埋め込まれた怪異を復元する調査怪談。',
    flavor:'民俗資料・土地・行政記録',start:'KRS-001「三本目の境木」',
    order:'各話は独立。公開順に読むと、黒瀬の宿泊室へ近づいてくる記録のつながりを拾えます。',
    promise:'資料提示だけで終わらず、現地調査、規則の特定、対処、その後の記録まで一話で描きます。'
  },
  '榊家異聞':{
    slug:'sakaki',english:'SAKAKI FAMILY ACCOUNTS',
    lead:'仏間、遺品、古い写真、家族だけの呼び名。家の中で共有されていた記憶が、人数と続柄を静かに組み替える家族怪談。',
    flavor:'家族・遺品・記憶',start:'SKK-001「仏間の夏服」',
    order:'各話は単独でも読めますが、家族の輪郭が徐々に変わるため第1話からの公開順を推奨します。',
    promise:'家族の感情と怪異の規則を分けて描き、各話の出来事には明確な対処と結果があります。'
  },
  '境界観測記':{
    slug:'kansoku',english:'BOUNDARY OBSERVATION LOG',
    lead:'佐伯冬真と御厨澪が、場所・記録・認識の境目に発生する案件を複数の観測手段で調査する連作怪談。',
    flavor:'調査チーム・境界現象・連作',start:'KKS-S1E01「境目の部屋」',
    order:'Season 1は第1話から第12話まで順番に読んでください。各事件は完結しますが、人物と青い避難所札の伏線が継続します。',
    promise:'各話の依頼、怪異、検証、対処、結果を完結させながら、全12話で一つの観測記録を形成します。'
  }
};
...
for(const [name,config] of Object.entries(configs)){
  ...
  const cards=group.map((work,index)=>`<a class="story-card" href="/kyokai-yawa/stories/${esc(work.file)}"><span class="story-number">${String(index+1).padStart(2,'0')}</span><div class="story-copy"><h3>${esc(work.title)}</h3><p>${esc(work.desc)}</p></div><div class="story-meta"><span>${esc(work.id)}</span><span>${esc(work.mins)}</span><span>恐怖 ${esc(work.fear)}</span></div></a>`).join('');
...
<section class="guide" aria-label="シリーズ案内"><article><h2>読む順番</h2><p>${esc(config.order)}</p></article><article><h2>一話の構成</h2><p>${esc(config.promise)}</p></article><article><h2>初めて読む方へ</h2><p>各カードには読了時間と恐怖レベルを表示しています。最初の一話から読む場合は、下の第1話を選んでください。</p></article></section>
<section id="stories" aria-labelledby="stories-title"><div class="section-heading"><h2 id="stories-title">全12話</h2><p>公開順</p></div><div class="story-grid">${cards}</div></section>
