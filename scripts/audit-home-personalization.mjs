...
if(index.indexOf('saved-stories.js')>index.indexOf('home-personalization.js'))errors.push('保存管理JavaScriptが個別化処理より後に読み込まれます');
if(index.indexOf('home-personalization.js')>index.indexOf('archive-tools.js'))warnings.push('個別化JavaScriptが一覧検索より後に読み込まれます');
...
const report=['# 境界夜話 再訪者向け個別入口監査','',`- 公開作品: ${works.length}/48`,'- 続きから読む: 保存位置が8〜93％の作品から最新1話','- 最近読んだ作品: 閲覧・読了時刻順で最大3話','- あとで読む: 保存順で最大3話','- 未読のおすすめ: 題材・シリーズ・時間・恐怖度から最大3話','- 履歴・保存がない初回訪問: 非表示','- 途中位置リンク: `?resume=1`で保存位置へ移動','- 保存方式: ブラウザー端末内localStorage','- 外部送信: なし',`- エラー: ${errors.length}`,`- 警告: ${warnings.length}`,'','## エラー','',...(errors.length?errors.map(error=>`- ${error}`):['- なし']),'','## 警告','',...(warnings.length?warnings.map(warning=>`- ${warning}`):['- なし']),''].join('\n');
