...
<meta name="description" content="この端末に保存された境界夜話の読了、途中位置、あとで読むを一覧で確認し、JSONでバックアップできます。">
...
<section class="backup-panel" data-reading-backup aria-labelledby="backup-title">
  <div class="backup-panel__head"><div><p class="eyebrow">LOCAL BACKUP</p><h2 id="backup-title">読書記録をバックアップ</h2></div><p>読了、閲覧履歴、あとで読む、途中位置、文字サイズをJSONファイルへ保存し、別の端末やブラウザーで復元できます。</p></div>
  <div class="backup-actions"><button type="button" data-backup-export>バックアップJSONを保存</button><button type="button" data-backup-share hidden>バックアップを共有</button><label class="backup-file"><span>JSONから復元</span><input type="file" data-backup-file accept=".json,application/json"></label></div>
  <p class="backup-privacy">ファイルには作品ID、閲覧・読了日時、途中位置などが含まれます。作品本文、氏名、メールアドレス、アカウント情報は含まれません。</p>
  <div class="backup-preview" data-backup-preview hidden>
    <h3>復元する内容</h3>
    <dl class="backup-preview__summary"><div><dt>書き出し日時</dt><dd data-preview-exported></dd></div><div><dt>読了済み</dt><dd data-preview-read></dd></div><div><dt>あとで読む</dt><dd data-preview-saved></dd></div><div><dt>途中位置</dt><dd data-preview-progress></dd></div><div><dt>閲覧履歴</dt><dd data-preview-history></dd></div><div><dt>文字サイズ</dt><dd data-preview-size></dd></div></dl>
    <fieldset class="backup-modes"><legend>復元方法</legend><label><input type="radio" name="backup-mode" value="merge" checked><span><strong>現在の記録へ追加</strong><small>読了と保存を統合し、新しい履歴と進んでいる途中位置を残します。</small></span></label><label><input type="radio" name="backup-mode" value="replace"><span><strong>バックアップ内容で置換</strong><small>この端末の現在の読書記録を削除して、バックアップの状態へ戻します。</small></span></label></fieldset>
    <div class="backup-confirm-actions"><button type="button" data-backup-confirm disabled>この内容を復元</button><button type="button" data-backup-cancel>選択を取り消す</button></div>
  </div>
  <p class="backup-message" data-backup-message aria-live="polite"></p>
</section>
...
<script src="/kyokai-yawa/data/reading-backup.js"></script>
